...
describe("basket tests", () => {
    ...
    it("render correct divs", () => {
        const wrapper = shallow(<Basket
            myBasket={[{id: "6", title: "Biscoff Blondie", imageURL: "www.somewhere.com/image3", price: 1300},
                {id: "6", title: "Milky Way Crispy Roll Blondie", imageURL: "www.somewhere.com/image3", price: 1300}]}
            myBasketTotal={2600} deleteItem={() => null}/>);
        expect(wrapper.find(".basket-wrapper").exists()).toBe(true);
        expect(wrapper.find(".basket-items-wrapper").at(0).exists()).toBe(true);
        expect(wrapper.find(".basket-image-div").at(0).contains(<img src={"www.somewhere.com/image3"}
                                                                     alt={"Biscoff Blondie"}/>)).toBe(true);
    ...
    it("onClick functions work as expected", () => {
        ...
        const wrapper = mount(<Basket
            myBasket={[{id: "6", title: "Biscoff Blondie", imageURL: "www.somewhere.com/image3", price: 13},
                {id: "6", title: "Milky Way Crispy Roll Blondie", imageURL: "www.somewhere.com/image3", price: 13}]}
            myBasketTotal={10} deleteItem={mockFunc}/>);
        wrapper.find(".delete-button").at(0).simulate("click");
        expect(mockFunc).toHaveBeenCalled();
        expect(wrapper.prop("myBasket")).toStrictEqual([{
            id: "6",
            title: "Milky Way Crispy Roll Blondie",
            imageURL: "www.somewhere.com/image3",
            price: 13
        }]);
    ...
    it("removeFromBasket function works as expected", () => {
        const wrapper = shallow(<Basket myBasketTotal={10} myBasket={[{
            id: "9",
            title: "Half and Half Box",
            imageURL: "www.somewhere.com/image3",
            price: 1500,
            options: {1: "none selected", 2: "none selected"}
        },
            {
                id: "10",
                ...
                imageURL: "www.somewhere.com/image3",
                price: 1800,
                options: {1: "none selected", 2: "none selected", 3: "none selected", 4: "none selected"}
            }]} deleteItem={() => null}/>);
